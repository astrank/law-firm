...
export default function Profesional(props) {
    return (
        <div className="flex flex-col gap-6 relative">
            ...
            <div className="aspect-h-2 aspect-w-3 xl:aspect-w-8 xl:aspect-h-6 w-full overflow-hidden">
                <Image src={props.img} layout="fill" objectFit="cover" alt="image" />
            </div>
            <div className="flex flex-col gap-1">
                <h3 className="text-xl font-bold">{props.nombre}</h3>
                <span className="text-sm text-gray-600 tracking-wider font-medium">{props.profesion}</span>
            </div>
            <p className="text-gray-600 leading-7 text-sm">{props.descripcion}</p>
